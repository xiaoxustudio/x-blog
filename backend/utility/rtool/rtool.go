...
func ImgText(width, height int, text string) (b []byte) {
	...
	for i := 0; i < len(text); i++ {
		...
		fontPosX := float64(width/textLen*i) + fontSize*0.5

		// 限制文字在图片内
		if fontPosX+fontSize > float64(width) {
			fontPosX = float64(width) - fontSize
		}
		if fontPosX < 0 {
			fontPosX = 0
		}
...
func loadFontFace(points float64) font.Face {
	fontPath := "F:/ex/ReactEx/x-blog/backend/utility/rtool/x.TTF"
	fontBytes, err := os.ReadFile(fontPath)
	if err != nil {
		log.Fatalf("无法读取字体文件 %s: %v", fontPath, err)
	}

	f, err := truetype.Parse(fontBytes)
	if err != nil {
		log.Fatalf("无法解析字体数据: %v", err)
	}
